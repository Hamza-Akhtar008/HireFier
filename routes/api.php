			...
			->group(function ($router) {
				...
				Route::controller(SocialController::class)
					->group(function ($router) {
						...
						Route::get('{provider}', 'getProviderTargetUrl')->name('auth.social.connect');
						Route::get('{provider}/callback', 'handleProviderCallback')->name('auth.social.connect.callback');
					});
			});
	...
	->group(function ($router) {
		...
		Route::middleware(['auth:sanctum'])
			->group(function ($router) {
				...
				Route::put('{id}/company', 'updatePostCompany')->name('posts.update.company');
