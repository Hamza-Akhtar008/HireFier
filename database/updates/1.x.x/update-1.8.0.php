...
try {
	...
	// languages
	if (!Schema::hasColumn('languages', 'russian_pluralization') && Schema::hasColumn('languages', 'script')) {
	...
	// currencies
	if (!Schema::hasColumn('currencies', 'decimal_places') && Schema::hasColumn('currencies', 'in_left')) {
		Schema::table('currencies', function (Blueprint $table) {
			$table->integer('decimal_places')->unsigned()->nullable()->default(2)->comment('Currency Decimal Places - ISO 4217')->after('in_left');
		});
	}
	if (!Schema::hasColumn('currencies', 'decimal_separator') && Schema::hasColumn('currencies', 'decimal_places')) {
		Schema::table('currencies', function (Blueprint $table) {
			$table->string('decimal_separator', 10)->nullable()->default('.')->after('decimal_places');
		});
	}
	if (!Schema::hasColumn('currencies', 'thousand_separator') && Schema::hasColumn('currencies', 'decimal_separator')) {
		Schema::table('currencies', function (Blueprint $table) {
			$table->string('thousand_separator', 10)->nullable()->default(',')->after('decimal_separator');
		});
	}
	
	DB::table('currencies')->update(['decimal_places' => 2]);
	DB::table('currencies')->update(['decimal_separator' => '.']);
	DB::table('currencies')->update(['thousand_separator' => ',']);
	
	// ads
	if (Schema::hasTable('ads')) {
		if (Schema::hasColumn('ads', 'salary_min')) {
			Schema::table('ads', function (Blueprint $table) {
				$table->decimal('salary_min', 10, 2)->nullable()->change();
			});
		}
		if (Schema::hasColumn('ads', 'salary_max')) {
			Schema::table('ads', function (Blueprint $table) {
	...
	if (!Schema::hasTable('pages')) {
		Schema::create('pages', function (Blueprint $table) {
			...
			$table->enum('type', ['standard', 'terms', 'privacy'])->default('standard');
