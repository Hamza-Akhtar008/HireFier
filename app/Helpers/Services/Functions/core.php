...
/**
 * Check if the "Multiple companies per user" option is enabled
 *
 * @return bool
 */
function isMultipleCompaniesPerUserEnabled(): bool
{
	return (config('settings.listing_form.multi_companies_per_user') == '1');
}
...
/**
 * When the "Multiple companies per user" option is enabled
 * Check if the company form step is enabled
 *
 * @param \Illuminate\Support\Collection|array|null $companies
 * @return bool
 */
function isCompanyFormEnabled(Collection|array|null $companies = []): bool
{
	// For guest user
	...
	// Get real user's companies
	$companies = $companies->reject(function ($item) {
		if (!isset($item['id'])) return true;
		
		return (empty($item['id']) || $item['id'] == 'new');
	});
	
	// Check if the logged-in user has one or many companies registered
	$userCompaniesNotFound = $companies->isEmpty();
	
	/*
	 * The company selection or creation form should be displayed in the following cases:
	 * - When the "Multiple Companies Per User" option is enabled.
	 * - When the logged-in user has not yet created a company.
	 * - When the current user is a guest (i.e., not logged in).
	 */
	
	return (isMultipleCompaniesPerUserEnabled() || $userCompaniesNotFound);
}
