...
class UserResource extends BaseResource
{
	...
	public function toArray(Request $request): array
	{
		...
		if (in_array('companies', $this->embed)) {
		...
		if (!empty($authUser)) {
			...
			// Logged User's Info
			if ($isAuthUserData) {
				if (in_array('resumes', $this->embed)) {
					$resumes = $this->whenLoaded('resumes');
					$resumesCollection = new EntityCollection(ResumeResource::class, $resumes, $this->params);
					$entity['resumes'] = $resumesCollection->toArray(request(), true);
				}
