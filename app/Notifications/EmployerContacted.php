...
class EmployerContacted extends BaseNotification
{
	...
	public function toMail($notifiable): MailMessage
	{
		...
		if (!empty($this->messageArray['file_path'])) {
			if (!empty($this->messageArray['file_data'])) {
				...
				if (str_starts_with($this->messageArray['file_path'], 'resumes/')) {
					$pDisk = StorageDisk::getDisk('private');
				}
				if ($pDisk->exists($this->messageArray['file_path'])) {
					$fileData = $pDisk->get($this->messageArray['file_path']);
				}
	...
	{
		$msg = trans('sms.post_employer_contacted_content', [
			'appName' => config('app.name'),
			'title'   => $this->post->title,
