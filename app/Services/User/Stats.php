...
trait Stats
{
	...
	protected function getStats($userId): JsonResponse
	{
		...
		// companies
		$companies = isMultipleCompaniesPerUserEnabled()
			? Company::query()->withoutAppends()->where('user_id', $userId)->count()
			: 1;
		
		// resumes
		$resumes = Resume::query()->withoutAppends()->where('user_id', $userId)->count();
