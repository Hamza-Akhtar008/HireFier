...
class ThreadService extends BaseService
{
	...
	public function store($postId, SendMessageRequest $request): JsonResponse
	{
		...
		// Get or Create a Resume
		if (!empty($authUser) && !empty($resumeId)) {
			...
			if (!isset($resumeInput['file_path'])) {
				$resumeInput['file_path'] = null;
			}
		...
		if (!empty($authUser) && !empty($post->user)) {
			...
			$message->file_path = $resume->file_path;
			$message->save();
			...
			if ($request->hasFile('resume.file_path')) {
				$file = $request->file('resume.file_path');
				$messageArray['file_path'] = $file->getClientOriginalName();
		...
		if (isset($messageArray['resume'])) {
			if (!is_string($messageArray['resume'])) {
