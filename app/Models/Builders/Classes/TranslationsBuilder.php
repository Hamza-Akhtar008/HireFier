	...
	public function where($column, $operator = null, $value = null, $boolean = 'and'): static
	{
		...
		if ($jsonMethodsAreAvailable) {
			...
			return parent::where(function (self $query) use ($column, $locale, $value, $masterLocale) {
				...
				$collation = null;
				if ($isDiacriticsEnabled) {
					$serverCollation = data_get(DBEncoding::getServerCharsetAndCollation(), 'collation');
					$connectionCollation = DBTool::getDatabaseConnectionInfo('collation');
					$collation = DBEncoding::isValidCharsetAndCollation() ? $connectionCollation : $serverCollation;
				}
