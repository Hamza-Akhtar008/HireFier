...
function getCompany(companyId) {
	...
	const logoFieldValueEl = document.getElementById('logoFieldValue');
	...
	if (!logoFieldEl || !logoFieldValueEl || !companyFieldsEl) {
		return;
	}
	
	if (companyId === 0 || companyId === '0' || companyId === '') {
		logoFieldEl.style.display = 'none';
		logoFieldValueEl.innerHTML = '';
		companyFieldsEl.style.display = '';
	} else {
		if (!companyIdEl || !companyFormLinkEl) {
			return;
		}
		...
		logoFieldValueEl.innerHTML = '<img src="' + selectedCompanyLogo + '">';
